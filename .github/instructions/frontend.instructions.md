...
- If you change trusted browser controls, resume-from-pause behavior, request tracing, or KTV playback coordination, run `py tools/browser_control_regression.py --base-url http://127.0.0.1:9000/ --ensure-server --output logs/browser-control-regression.json` and expect `summary.passed = true`.
