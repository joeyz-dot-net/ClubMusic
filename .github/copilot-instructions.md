...
- **Backend**: FastAPI in [app.py](app.py) (~2018 lines, 60+ routes) built around module-level singletons: `PLAYER = MusicPlayer.initialize()`, `PLAYLISTS_MANAGER = Playlists()`, `RANK_MANAGER = HitRank()`, `SETTINGS = initialize_settings()`, `CURRENT_PLAYLIST_ID`. Business logic lives in [models/](models/) (player.py ~1522 lines, song.py ~487 lines, stream.py ~1434 lines). No dependency injection—all state is global and auto-persisted.
...
- **Audio streaming (FFmpeg)**: [models/stream.py](models/stream.py) (~1434 lines) provides browser audio stream via /stream/play endpoint. **Critical 2024 optimization**: 3-thread async non-blocking broadcast (read_stream + broadcast_worker + send_heartbeats threads) with ClientPool management, BROADCAST_QUEUE, and sequence numbers for packet loss detection. FFmpeg parameters optimized for low-latency (rtbufsize 8M, thread_queue_size 256, bufsize 64K). Browser-specific queue sizing (Safari 512 blocks = 128MB, Chrome/Edge 64 blocks = 16MB). Keepalive via sequence-numbered packets (seq_id < 0). Format-specific optimizations: MP3 (192KB chunks, 1s heartbeat), AAC (128KB chunks, 0.5s heartbeat), FLAC (256KB chunks, 1s heartbeat). See [doc/CHANGELOG.md](doc/CHANGELOG.md) for v2.0 streaming architecture details.
...
- **Config**: settings.ini read once at startup in [models/player.py](models/player.py) via `MusicPlayer.ensure_ini_exists()` and `from_ini_file()`. Fallback to `MusicPlayer.DEFAULT_CONFIG`. Key fields: `[app]` music_dir (Z:), allowed_extensions (.mp3,.wav,.flac,.aac,.m4a), server_host/server_port, debug, mpv_cmd, enable_stream (false=disable /stream endpoints), default_stream_format (aac/mp3/flac). Optional [paths] audio_input_device for FFmpeg (auto-detects via dshow if empty). Changes require full restart (reload=False in uvicorn). Format-specific configs in [formats] section (codec, bitrate, profile, chunk_size, heartbeat_interval, queue_multiplier). Browser overrides in [browser_format_overrides] (e.g., safari_aac = 768,128,15).
...
- **FFmpeg audio device missing** (CRITICAL): If /stream/play shows "音频设备名称为空" error, FFmpeg cannot find audio input device. Verify: (1) VB-Cable installed (https://vb-audio.com/Cable/); (2) [paths] audio_input_device in settings.ini matches system device name OR leave empty for auto-detection; (3) use `ffmpeg -list_devices true -f dshow -i dummy` to list available devices. Startup logs provide device list automatically.
...
- **Streaming disabled**: Setting enable_stream=false in settings.ini [app] section disables all /stream endpoints (returns 403). Check settings.ini [app] enable_stream value before debugging stream issues.
...
- **Streaming audio cuts out (Safari/Chrome)**: Verify three threads in stream.py (read_stream, broadcast_worker, send_heartbeats) active. Check get_keepalive_chunk() format support (MP3/AAC/FLAC). See [doc/CHANGELOG.md](doc/CHANGELOG.md) for v2.0 streaming architecture. Verify enable_stream=true in settings.ini [app] section.
...
## More references
- See [README.md](README.md) for feature overview and setup instructions. Detailed issue tracking and optimization history in [doc/](doc/) folder (CHANGELOG.md covers 2024 streaming architecture v2.0 improvements).